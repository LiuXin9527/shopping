...
function nodeitem( parentkey , key , lable , img )  {
    ...
    if( this.parent != null )  { //第一次註冊復節點 第二次子節點
        aa = this.parent.status ;
        if( aa.substring( 1 , 2 ) == "0" ) //第一次配置this.parent.status
            this.parent.status = aa.substring( 0 , 1 ) + "1" + aa.substring( 2 , 3 ) ; //parent.status=1+1+0
        if( this.parent.maxsubitem != null ) {
            this.parent.maxsubitem.status = "2" + this.parent.maxsubitem.status.substring( 1 , 3 ) ;//200
        }
        this.parent.subitems[ this.parent.subitems.length ] = this ; // 總共多少節點
        this.parent.maxsubitem = this ;  //最後一個節點物件
        //alert(this.parent.subitems.length);
    }
    else  {
        ...
        tv_topnodeitem = this ; //parent為topnodeitem
    }
    ...
    this.id = nodeitemRegister( this ) ; //註冊節點
...
function findNode( key )   {
    ...
    for( i = 0 ; i < nodeitems.length ; i ++ ) {
       if( nodeitems[ i ] != null ) {
          if( nodeitems[ i ].key == key ) {  // nodeitems[ i ].key = -1 == -1
             pppp = nodeitems[ i ] ;
          }
       }
    }
